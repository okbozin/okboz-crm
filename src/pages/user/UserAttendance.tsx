...
export default function UserAttendance({ isAdmin = false }: UserAttendanceProps) {
  ...
  const loadData = () => {
    try {
        ...
        if (isSuperAdmin) {
            ...
            if (isAdmin) {
                // If it's a Corporate Admin logged in (isAdmin is passed true, and not super admin)
                // Note: isAdmin prop is true for /admin/attendance route
                ownerId = sessionId;
            } else {
                // It is an Employee View (/user)
                const storedCorpId = localStorage.getItem('logged_in_employee_corporate_id');
                
                // Strict validation of stored ID to prevent [object Object] or null strings
                if (storedCorpId && storedCorpId !== 'null' && storedCorpId !== 'undefined' && storedCorpId !== '[object Object]') {
                    ownerId = storedCorpId;
                } else {
                    // Fallback: Search for the employee to find their owner (Self-Healing)
                    const empId = sessionId;
                    
                    // Check Admin Staff List first
                    const adminStaff = JSON.parse(localStorage.getItem('staff_data') || '[]');
                    if (adminStaff.some((e: any) => e.id === empId)) {
                        ownerId = 'admin';
                    } else {
                        // Check Corporate Staff Lists
                        for (const c of corps) {
                             const cStaff = JSON.parse(localStorage.getItem(`staff_data_${c.email}`) || '[]');
                             if (cStaff.some((e: any) => e.id === empId)) {
                                 ownerId = c.email;
                                 break;
                             }
                        }
                    }
                    // If still not found, default to 'admin' as fail-safe or use sessionId if it looks like a corporate ID
                    if (!ownerId) {
                        ownerId = 'admin'; 
                    }
                    
                    // Fix the stored value for future loads
                    localStorage.setItem('logged_in_employee_corporate_id', ownerId);
                }
            }

            // Now load data based on resolved ownerId
            const safeOwnerId = String(ownerId);
            
            let staffKey = `staff_data_${safeOwnerId}`;
            let branchKey = `branches_data_${safeOwnerId}`;

            // Handle Admin/Head Office case specifically
            if (safeOwnerId === 'admin') {
                staffKey = 'staff_data';
                branchKey = 'branches_data';
            }

            const cStaff = JSON.parse(localStorage.getItem(staffKey) || '[]');
            const cBranches = JSON.parse(localStorage.getItem(branchKey) || '[]');
            
            staff = cStaff.map((s: any) => ({...s, corporateId: safeOwnerId}));
            branches = cBranches.map((b: any) => ({...b, owner: safeOwnerId}));
        }
        setEmployees(staff);
  ...
  // Check restrictions - Logic: if mode is 'BranchRadius', Remote is restricted.
                                ...
                                {dailyReportData.map(({employee, record}) => (
                                    <tr key={employee.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-6 py-3 font-medium text-gray-900">
                                            {employee.name}
                                            <div className="text-xs text-gray-400 font-normal">{String(employee.role || '-')}</div>
                                        </td>
                                        <td className="px-6 py-3 text-gray-600">{String(employee.branch || '-')}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                  ...
                  <div className="p-6 space-y-4">
                      {/* Grid column logic: 1 col if restricted, 2 cols if not */}
                      ...
                      {/* Info Text if Restricted */}
                      {isBranchRestricted && (
                          <div className="text-xs text-center text-gray-400 bg-gray-50 p-2 rounded border border-gray-100">
                             Remote punch is disabled by your administrator.
                          </div>
                      )}
